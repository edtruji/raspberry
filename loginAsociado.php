...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['passcode'])) {
    $passcode = (int)$_POST['passcode']; // Cast to integer
    
    try {
        // Connect to SQLite database
        $db = new SQLite3('amusement.db');
        
        // Prepare and execute query
        $stmt = $db->prepare('SELECT users FROM users WHERE passcode = :passcode');
        $stmt->bindValue(':passcode', $passcode, SQLITE3_INTEGER); // Bind as INTEGER
        $result = $stmt->execute();
